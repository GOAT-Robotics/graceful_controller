...
namespace graceful_controller
{
  ...
  */
      ...
      std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
  {
    ...
    declare_parameter_if_not_declared(node, name_ + ".odom_enable", rclcpp::ParameterValue(false));
    ...
    node->get_parameter(name_ + ".odom_enable", odom_enable_);
    ...
    RCLCPP_INFO(LOGGER, "  odom_enable: %s", odom_enable_ ? "TRUE" : "FALSE");
    ...
    for (int i = static_cast<int>(transformed_global_plan_poses.size()) - 1; i >= 0; --i)
    {
      ...
      do
      {
        ...
        if (simulate(target_pose, velocity, cmd_vel))
        {
          ...
          // Both the yaw hold (angular.z=0.0) and yaw correction (P-controller) branches
          // override the simulation's path-following steering, causing the robot to drift
          // away from the goal. The simulation already handles yaw alignment via the
          // target_pose orientation set on lines 773-776.
          // Pending senior review for permanent removal.
          //
          ...
          //   {
          //     cmd_vel.twist.angular.z = 0.0;
          //     RCLCPP_INFO(LOGGER, "Moving yaw hold -> ...");
          //   }
          //   else
          //   {
          ...
          //     RCLCPP_INFO(LOGGER, "Moving yaw correction -> ...");
          //   }
          // }
  ...
  void GracefulControllerROS::setPlan(const nav_msgs::msg::Path &path)
  {
    ...
    // If odom_enable is true, transform the path to the odom frame (like docking controller)
    nav_msgs::msg::Path working_plan;
    if (odom_enable_)
    {
      try
      {
        ...
        for (const auto &pose : path.poses)
        {
          ...
          working_plan.poses.push_back(transformed_pose);
        }
        working_plan.header = path.header;
        working_plan.header.frame_id = "odom";
        RCLCPP_INFO(LOGGER, "Odom mode enabled: transformed plan to odom frame with %lu poses.", working_plan.poses.size());
      }
      catch (tf2::TransformException &ex)
      {
    ...
    if (compute_orientations_)
    {
      ...
      oriented_plan = addOrientations(working_plan);
    }
    else
    {
      RCLCPP_INFO(LOGGER, "Using plan without adding orientations.");
      oriented_plan = working_plan;
    }
    ...
    if (backward_motion_available_)
    {
      ...
      bool got_robot_orientation = false;

      if (odom_enable_)
      {
        try
        {
          geometry_msgs::msg::TransformStamped transformStamped = buffer_->lookupTransform(
              filtered_plan.header.frame_id,
              costmap_ros_->getBaseFrameID(),
              tf2::TimePointZero);

          tf2::Quaternion orientation;
          tf2::fromMsg(transformStamped.transform.rotation, orientation);
          double roll, pitch, yaw;
          tf2::Matrix3x3(orientation).getRPY(roll, pitch, yaw);
          robot_orientation = normalizeAngle(yaw);
          got_robot_orientation = true;
        }
        catch (tf2::TransformException &ex)
        {
          RCLCPP_ERROR(LOGGER, "Failed to get robot pose in plan frame for backward motion check: %s", ex.what());
        }
      }
      else
      {
        // Callback spin to get robot pose (original behavior)
  ...
  void GracefulControllerROS::publishDistanceToGoal()
  {
    ...
    // Skip if no plan or not initialized
    if (!initialized_ || global_plan_.poses.empty())
    {
      return;
    }

    // Check if the controller is still being ticked (computeVelocityCommands called)
    // If not ticked recently, skip publishing
    auto now = clock_->now();
    double seconds_since_last_tick = (now - last_ticked_time_).seconds();
    if (seconds_since_last_tick > 1.0)
    {
      return;
    }
    ...
    try
    {
      plan_to_robot = buffer_->lookupTransform(
        costmap_ros_->getBaseFrameID(),
        global_plan_.header.frame_id,
        tf2::TimePointZero);
    }
    ...
    {
      RCLCPP_DEBUG(LOGGER, "publishDistanceToGoal: Could not transform: %s", ex.what());
      return;
    }

    // Transform all plan poses to robot frame
    std::vector<geometry_msgs::msg::PoseStamped> transformed_poses;
    transformed_poses.reserve(global_plan_.poses.size());
    for (const auto &pose : global_plan_.poses)
    {
      geometry_msgs::msg::PoseStamped tp;
      tf2::doTransform(pose, tp, plan_to_robot);
      transformed_poses.push_back(tp);
    }

    // Compute distances along path
    std::vector<double> distances;
    computeDistanceAlongPath(transformed_poses, distances);
    ...
    // Distance to goal is the last entry (cumulative distance along the path)
    double dist = distances.back();

    std_msgs::msg::Float32 msg;
    msg.data = static_cast<float>(dist);
    distance_to_goal_pub_->publish(msg);
  }
